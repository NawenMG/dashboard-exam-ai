...
async def compute_final_grade(
    ...
    return await FinalGradeService.compute_and_save(
        db, teacher=teacher, payload=payload
    )


@router.get("/by-submission/{submission_id}", response_model=FinalGradeOut)
async def get_final_grade_by_submission(
    submission_id: int,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
):
    return await FinalGradeService.get_by_submission_id(
